...
clone() {
  ...
  if [ ! -d "$into" ]; then
    mkdir -p "$into"
    git clone -q "https://$repo" "$into"
  fi
  (cd "$into" && git clean -fdxq && git fetch -q && git reset -q --hard "$ref")
}
...
EOF
    go run github.com/davecheney/godoc2md -v -template scripts/go-pkg.md "$repo" >> "$basedir/$name/godoc.md"
    ;;
  js-*|interface-js-*)

    # JS: clone repo, npm install, run aegir docs
    tmpdir="tmp/js"
    clone "$repo" "$tmpdir/$name" "$ref"
